...
#form h5{
 ...
 <span><input type="button" value="预览" class="button" onclick = "preview()">
		...
		<span id = "_book" style="display:none">敬设:<input style="width:150px;" type="text" id = "book" name="book" value="《药师经》一昶日" ><br><br></span>
			...
			<c:forEach items="${pharmacistbirthEntitys}" var="pharmacistbirthEntity" varStatus="stauts">
				<tr>
					<td><input style="width:20px;"  type="checkbox"/></td>
				 	<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${pharmacistbirthEntity.prayingobj}"></td>
				 	<td onclick="getSelected(this)"><input type="text" name="livingmenber" id = "livingmenber" value="${pharmacistbirthEntity.livingmenber}"></td>
				 	<%-- <td><input type="text" id = "address" name="address" value="${pharmacistbirthEntity.address}"></td> --%>
				 	<input type="hidden" id = "address" name="address" value="${pharmacistbirthEntity.address}">
<%-- 				 	<input type="hidden" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress:pharmacistbirthEntity.address}"> --%>
				 	<td><input style="width:40px;" type="text" id = "money" name="money" value="${pharmacistbirthEntity.money}" required></td>
				 	<td>
					...
					<select style="width:50px;" id="size" name="size" onchange="changeSize(this.value);">
						<option value="小" <c:if test="${pharmacistbirthEntity.size eq '小'}">selected="selected"</c:if>>小</option>
							<option value="中" <c:if test="${pharmacistbirthEntity.size eq '中'}">selected="selected"</c:if>>中</option>
							<option value="大" <c:if test="${pharmacistbirthEntity.size eq '大'}">selected="selected"</c:if>>大</option>
					</select>
					</td>
				 	
				 	<td><textarea rows="3" cols="60" id="summary" name="summary" required>${pharmacistbirthEntity.summary}</textarea>
					</td>
