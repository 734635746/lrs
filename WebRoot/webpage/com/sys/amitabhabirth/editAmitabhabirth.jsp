	...
	//获取复选框选中的值
 	...
 	function changSumary(currentNode){
		...
		var prayingobj=currentNode.parentNode.parentNode.children[1].children[0].value;
				...
				if(index == 0){//十方法界
				//currentNode.textContent = "农历${funeralheld[0].holdDate} 弥陀诞法会。十方法界";
...
#form h5{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<input type="button" value="添加记录" id = "buttonAdd" class="button" onclick="addTr()">
 		<input type="button" value="撤销记录" id = "buttonDel" class="button" onclick="deteleTr()">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${amitabhabirthEntitys}" var="amitabhabirthEntity" varStatus="stauts">
				<tr>
					...
					<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${amitabhabirthEntity.prayingobj}"></td>
				 	<td><select style="width:100px;" id="type" name="type" onchange="change(this.value,this)">
						<option value="2" ${amitabhabirthEntity.type == 2 ? 'selected = "selected"' : '' }>十方法界</option>
						<option value="3" ${amitabhabirthEntity.type == 3 ? 'selected = "selected"' : '' }>门堂历代宗亲</option>
						<option value="4" ${amitabhabirthEntity.type == 4 ? 'selected = "selected"' : '' }>堕胎婴灵</option>
						<option value="1" ${amitabhabirthEntity.type == 1 ? 'selected = "selected"' : '' }>无</option>
					</select>
					</td>
				 	<td onclick='getSelected(this)'><input type="text" name="ancestor" id = "ancestor" value="${amitabhabirthEntity.ancestor}"></td>
				 	<%-- <td><input type="text" id = "address" name="address" value="${amitabhabirthEntity.address}"></td> --%>
				 	<%-- <input type="hidden" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress: amitabhabirthEntity.address}"> --%>
				 	<input type="hidden" id = "address" name="address" value="${amitabhabirthEntity.address}">
					...
					<td>
					<input style="width:50px; type = "text" value = "${amitabhabirthEntity.size}" name = "size" readonly> <!-- 8月7号 -->
					</td>
				 	
				 	<td><textarea rows="3" cols="60" id="summary" name="summary" required onclick="changSumary(this);">${amitabhabirthEntity.summary}</textarea>
					</td>
    	...
    	<center>
			...
			<table id = "receipttable" class="imagetable" width="100%">
				<tr>
					<th>选择</th>
					<th>阳上</th>
					<th>超渡类型</th>
				...
				<tr>
					 ...
					 <input type="hidden" id = "address" name="address" value="${ doritualinfoAddress}">
						...
						<td  width="10px">
						<input style="width:50px; type = "text" value = "${size}" name = "size"  readonly> <!-- 8月7号 -->
						</td>
