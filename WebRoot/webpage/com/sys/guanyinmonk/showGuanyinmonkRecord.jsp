 ...
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	function toGuanyinmonk(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/guanyinmonkController.do?redirectToEditGuanyinmonk&id=" + id0 + "&size=" + encodeURI(size);
  	}
  	...
  	function selectBox(){
		var count = 0;
		 ...
		 $("[name='checkbox']:checked").each(function(){
			 guanyinmonkIds += $(this).val()+",";
			 count = count + 1;
		  });
		 
		 if(document.getElementById("size").value == "小"){
			 if(guanyinmonkIds == "" || count > 5){
				 alert("请选择至少一个牌位，至多5个牌位");
				 return false;
			 }
			 else{
				 var id0 = document.getElementById("id").value;
				 window.location.href="${pageContext.request.contextPath}/guanyinmonkController.do?redirectToEditGuanyinmonk&id=" + id0 + "&guanyinmonkIds=" + guanyinmonkIds + "&size=" + encodeURI(size);
			 }	 
		 }
		 else{
			 if(guanyinmonkIds == "" || count > 1){
				 alert("请选择一个牌位");
				 return false;
			 }
			 else{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${guanyinmonkEntitys}" var="guanyinmonkEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${guanyinmonkEntity.id}" name="checkbox"></td>
					<td>${stauts.index+1}</td>
					<td>${guanyinmonkEntity.paymen}</td>
					<td>${guanyinmonkEntity.serial}</td>
					<td>${guanyinmonkEntity.size}</td>
					<%-- <td>${guanyinmonkEntity.receiptno}</td> --%>
				 	<td>${guanyinmonkEntity.registertime}</td>
				 	<td>${guanyinmonkEntity.prayingobj}</td>
				 	<td>${guanyinmonkEntity.livingmenber}</td>
				 	<td>${guanyinmonkEntity.address}</td>
				 	<td>${guanyinmonkEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty guanyinmonkEntitys}">
			<span id="recordArea">没有观音出家记录，请点击<input type="button" value="观音出家登记" class="button" onclick="toGuanyinmonk()">&nbsp;&nbsp;</span>
			</c:if>
		</center>
 </body>