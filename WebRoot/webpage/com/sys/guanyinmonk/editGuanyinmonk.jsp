...
#form h5{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<c:if test="${!empty guanyinmonkEntitys}">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${guanyinmonkEntitys}" var="guanyinmonkEntity" varStatus="stauts">
				<tr>
					<td><input style="width:20px;"  type="checkbox"/></td>
				 	<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${guanyinmonkEntity.prayingobj}"></td>
				 	<td onclick="getSelected(this)"><input type="text" name="livingmenber" id = "livingmenber" value="${guanyinmonkEntity.livingmenber}"></td>
				 	<%-- <td><input type="text" id = "address" name="address" value="${guanyinmonkEntity.address}"></td> --%>
				 	<input type="hidden" id = "address" name="address" value="${guanyinmonkEntity.address}">
<%-- 				 	<input type="hidden" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress:guanyinmonkEntity.address}"> --%>
				 	<td><input style="width:40px;" type="text" id = "money" name="money" value="${guanyinmonkEntity.money}" required></td>
				 	<td>
					...
					<select style="width:50px;" id="size" name="size" onchange="changeSize(this.value);">
						<option value="小" <c:if test="${guanyinmonkEntity.size eq '小'}">selected="selected"</c:if>>小</option>
					</select>
					</td>
				 	
				 	<td><textarea rows="3" cols="60" id="summary" name="summary" required>${guanyinmonkEntity.summary}</textarea>
					</td>
