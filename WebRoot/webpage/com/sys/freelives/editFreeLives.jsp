...
<script type="text/javascript">
	...
	//$("#lunardate").val(lunar.lYear + '年' +lunar.IMonthCn+lunar.IDayCn);
		...
		//选择日期后显示当天的随堂和包堂数
        $.ajax({
            url:"${pageContext.request.contextPath}/freeLivesController.do?getCount",
...
/*				current.value = window.temp;
	...
	function changSumary(currentNode){
		...
		for (var i = 1; i < rowcount; i++) {
			...
			temp+=date.substring(3, date.length);
			temp += tr[i].getElementsByTagName("td")[4].children[0].value + " ";
			temp += tr[i].getElementsByTagName("td")[5].children[0].value +"元、";
		}
 	...
 	function changMoney(){
		...
		for (var i = 1; i < rowcount; i++) {
			if(type == "随堂"){
				tr[i].getElementsByTagName("td")[5].children[0].value = 600;
			}else if(type == "包堂"){
				tr[i].getElementsByTagName("td")[5].children[0].value = 6000;
			}
		}
 	}
...
<body style="overflow-y: scroll" scroll="yes">
	...
	<center>
		...
		<c:if test="${!empty morningpforrEntitys}">
			<form
				action="${pageContext.request.contextPath}/freeLivesController.do?getSerialAndSaveTablet"
				...
				<table id="receipttable" class="imagetable">
					...
					<c:forEach items="${morningpforrEntitys}" var="morningpforrEntity"
						...
						<tr>
							...
							<!-- 付款人 -->
							<td><input style="width:90px;" type="text" id="prayingobj"
								name="prayingobj" value="${morningpforrEntity.prayingobj}">
							</td>
							<!-- 敬设 -->
							<%-- <td><input style="width:80px;" type="hidden" id="book"
								name="book" value="${morningpforrEntity.book}">
							</td> --%>
							<input style="width:80px;" type="hidden" id="book"
								name="book" value="无">
							<!-- 公历 -->
							...
							<td><input type="text" id="lunardate" name="lunardate"
								...
								随堂：<label id="stCount">${counts[stauts.index].st}</label>&nbsp;&nbsp;&nbsp;&nbsp;
								包堂：<label id="btCount">${counts[stauts.index].bt}</label></td>
							<!-- 信众 -->
							<td><input  type="text" maxlength="8" placeholder="请输入最多8个字符"
...
<%-- 							<td><input type="text" id="address" name="address"
					...
					<select style="width:50px;" id="size" name="size" onchange="changeSize(this.value);">
						<option value="小" <c:if test="${guanyinopenEntitys.size eq '小'}">selected="selected"</c:if>>小</option>
							...
							<!-- 摘要 -->
							<td><textarea rows="4" cols="70" id="summary" name="summary"
				...
				<table id="receipttable" class="imagetable">
					...
					<tr>
						...
						<!-- <td><input style="width:100px;" type="text" id="book"
							name="book" value="《祈福消灾延寿普佛一堂》" required> -->
							<input style="width:100px;" type="hidden" id="book"
							name="book" value="无" required>
							 <!-- 公历 -->
						<td><input type="date" id="solardate" name="solardate"
						...
						<td><input style="width:40px;" type="text" id="money"
							name="money" value="500" required ><!-- readonly="readonly" -->
							</td>
						<td><select style="width:60px;" id="payway" name="payway">
								<option value="现金">现金</option>
								<option value="刷卡">刷卡</option>
