 ...
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	function toJIzobirth(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/jIzobirthController.do?redirectToEditJizobirth&id=" + id0 + "&size=" + encodeURI(size);;
  	}
  	...
  	function selectBox(){
		 ...
		 if(document.getElementById("size").value == "小"){
			 if(jizobirthIds == ""){
				 ...
				 var id0 = document.getElementById("id").value;
				 window.location.href="${pageContext.request.contextPath}/jIzobirthController.do?redirectToEditJizobirth&id=" + id0 + "&jizobirthIds=" + jizobirthIds + "&size=" + encodeURI(size);;
			}	
		 }
		 else{
			 if(jizobirthIds == "" || count > 1){
				 alert("请选择一个牌位");
				 return false;
			 }
			 else{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${jIzobirthEntitys}" var="jIzobirthEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${jIzobirthEntity.id}" name="checkbox"></td>
					<td>${stauts.index+1}</td>
					<td>${amitabhabirthEntity.paymen}</td>
					<td>${jIzobirthEntity.serial}</td>
					<td>${jIzobirthEntity.size}</td>
					<%-- <td>${jIzobirthEntity.receiptno}</td> --%>
				 	<td>${jIzobirthEntity.registertime}</td>
				 	<td>${jIzobirthEntity.prayingobj}</td>
				 	...
				 	<td>${jIzobirthEntity.address}</td>
				 	<td>${jIzobirthEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty jIzobirthEntitys}">
			<span id="recordArea">没有地藏诞记录，请点击<input type="button" value="地藏诞登记" class="button" onclick="toJIzobirth()">&nbsp;&nbsp;</span>
			</c:if>
		</center>
 </body>