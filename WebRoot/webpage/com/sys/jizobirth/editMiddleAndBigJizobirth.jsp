...
#form h5{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		<h4>编辑地藏诞${size}牌记录</h4><input type="button" value="预览" class="button" onclick = "preview()">
		...
		<form action="${pageContext.request.contextPath}/jIzobirthController.do?getSerialAndSaveTablet" onsubmit="return myvalidate()" method="post">
		
		<input id="id" name="id" type="hidden" value="${id}">
		<input id="size" name="sizeflag" type="hidden" value="${size}">
		<input type = "hidden" value = "${jIzobirthid}" name = "jIzobirthid" id = "jIzobirthid">
		...
		<td>
			...
			<tr>
				<th>付款人</th>
				<td><input style="width:100px;" type="text" id = "paymen" name="paymen" value="${jIzobirthEntity.paymen}" required> 
				</td>
			</tr>
			<tr>
				<th>敬设</th>
				<td><input style="width:150px;" type="text" id = "book" name="book" value="《地藏经》一昶日"  required> 
				</td>
			</tr>
			<tr>
				<th>阳上</th>
				<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${jIzobirthEntity.prayingobj}" required>
				</td>
			</tr>
			<tr>
				<th>付款方式</th>
				<td> <select style="width:60px;" id="payway" name="payway" >
								<option value="现金" ${jIzobirthEntity.payway == '现金' ? 'selected = "selected"' : '' }>现金</option>
								<option value="刷卡" ${jIzobirthEntity.payway == '刷卡' ? 'selected = "selected"' : '' }>刷卡</option>
								<option value="支付宝" ${jIzobirthEntity.payway == '支付宝' ? 'selected = "selected"' : '' }>支付宝</option>
								<option value="微信" ${jIzobirthEntity.payway == '微信' ? 'selected = "selected"' : '' }>微信</option>
								<option value="其他" ${jIzobirthEntity.payway == '其他' ? 'selected = "selected"' : '' }>其他</option>
						</select>
				</td>
			</tr>
			<tr>
				<th>地址</th>
				<td> <input type="text" style="width:400px;" id = "address" name="address" value="${jIzobirthEntity.address}"></td>
<%-- 				<td> <input type="text" style="width:250px;" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress:jIzobirthEntity.address}"></td> --%>
			</tr>
			<%-- <input type="hidden" style="width:250px;" id = "address" name="address" value="${ doritualinfoAddress}"> --%>
			<tr>
				<th>金额</th>
				<td> <input type="text" style="width:60px;" id = "money" name="money" value="${jIzobirthEntity.money}" required onclick="changMoney();"></td>
			</tr>
			<tr>
				<th>摘要</th>
				<td> <textarea rows="3" cols="60" id="summary" name="summary"   onclick="changSumary()" autofocus required>${jIzobirthEntity.summary}</textarea></td>
			</tr>
    	...
    	<center>
    		<c:if test="${empty jIzobirthEntitys}">
    		<form action="${pageContext.request.contextPath}/jIzobirthController.do?getSerialAndSaveTablet" onsubmit="return myvalidate()" method="post">
			<input id="id" name="id" type="hidden" value="${id}">
		<input id="size" name="sizeflag" type="hidden" value="${size}">
		<table class="imagetable" width="1000px">
			...
			<table id = "receipttable" class="imagetable" width="500px">
				...
				<tr>
						...
						<td style="width:30px; text-align:center;">5</td>
					 	<td><input style="width:100%;" type="text" name="ancestor" id = "ancestor" value="" placeholder="请在此处输入堕胎婴灵，最多8个字符"></td>
				</tr>
				<tr>
						<!-- 添加标号 -->
