...
#form h5{
 ...
 <input type="button" value="预览" class="button" onclick = "preview()">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${jIzobirthEntitys}" var="jIzobirthEntity" varStatus="stauts">
				<tr>
					<td><input style="width:20px;"  type="checkbox"/></td>
				 	<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${jIzobirthEntity.prayingobj}"></td>
				 	<td><select style="width:100px;" id="type" name="type" onchange="change(this.value,this)">
						<option value="2" ${jIzobirthEntity.type == 2 ? 'selected = "selected"' : '' }>十方法界</option>
						<option value="3" ${jIzobirthEntity.type == 3 ? 'selected = "selected"' : '' }>门堂历代宗亲</option>
						<option value="4" ${jIzobirthEntity.type == 4 ? 'selected = "selected"' : '' }>堕胎婴灵</option>
						<option value="1" ${jIzobirthEntity.type == 1 ? 'selected = "selected"' : '' }>无</option>
					</select>
					</td>
				 	<td onclick="getSelected(this)"><input type="text" name="ancestor" id = "ancestor" value="${jIzobirthEntity.ancestor}"></td>
				 	<%-- <td><input type="text" id = "address" name="address" value="${jIzobirthEntity.address}"></td> --%>
				 	<input type="hidden" id = "address" name="address" value="${jIzobirthEntity.address}">
<%-- 				 	<input type="hidden" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress:jIzobirthEntity.address}"> --%>
					...
					<select style="width:50px;" id="size" name="size" >
				 	...
				 	<td><textarea rows="3" cols="60" id="summary" name="summary" required onclick="changSumary(this);">${jIzobirthEntity.summary}</textarea>
					</td>
