  ...
  <script type="text/javascript"> 
	...
	//获取复选框选中的值
 	...
 	function myvalidate(){
 		...
 		for(var i = 1 ;i < rowcount;i ++){
 			if (!(/(^[1-9]\d*$)/.test(tr[i].getElementsByTagName("td")[8].children[0].value))){
 	 ...
 	 function changeSolarToLunar(currentNode){
 	 	...
 	 	//选择日期后显示当天的随堂和包堂数
        $.ajax({
            url:"${pageContext.request.contextPath}/morningpforrController.do?getCount",
	...
	function getAncestors(current) {
		...
		if(type == "1"){//十方法界
			dlg = $.messager.show({
				id : "livings1", //窗口ID
				title : '输入超度对象',
				msg : '<div style="" margin-left: 0px margin-top: 0px align="center"><table id = "livingmenbertable" class="imagetable" width="500px"><tr><td><input style="width:100%;" type="text" name="livingmember1" id = "livingmember1" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember2" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember3" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember4" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember5" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember6" value="" placeholder="请输入最多8个字符"></td></tr></table><div><input id="confirm1" type="button" class="button" value="确定""></div></div>',
				showType : 'fade',
				timeout : 0,
				closable : true,
				modal : true,
			...
			dlg = $.messager.show({
				id : "livings2", //窗口ID
				title : '输入超度对象',
				msg : '<div style="" margin-left: 0px margin-top: 0px align="center"><table id = "livingmenbertable" class="imagetable" width="500px"><tr><td><input style="width:100%;" type="text" name="livingmember1" id = "livingmember1" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember2" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember3" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember4" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember5" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember6" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember7" value="" placeholder="请输入最多8个字符"></td></tr><tr><td><input style="width:100%;" type="text" name="livingmember" id = "livingmember8" value="" placeholder="请输入最多8个字符"></td></tr></table><div><input id="confirm2" type="button" class="button" value="确定""></div></div>',
				showType : 'fade',
				timeout : 0,
				closable : true,
				modal : true,
...
//
		$(document).on('click', '#confirm1', function() {
			...
			//document.getElementById("ancestor").value = livingMen;
			//console.log($(".ancestor"));
			window.temp=livingMen;
			//$(".ancestor").attr("value",livingMen);
			dlg.window('close');
			//console.log(input);
	...
	function changSumary(currentNode){
		...
		sumary += (date + "作往生普佛共" + (rowcount - 1) + "堂");
	...
	function changMoney(){
		...
		for (var i = 1; i < rowcount; i++) {
			if(size == "随堂"){
				tr[i].getElementsByTagName("td")[8].children[0].value = 560;
			}else if(size == "包堂"){
				tr[i].getElementsByTagName("td")[8].children[0].value = 6000;
			}
		}
 	}
  	</script>
...
#address{
 ...
 <h4>编辑晚普佛记录</h4>
  		...
  		<c:if test="${empty morningpforrEntity}"><!-- 编辑的时候不需要添加功能 -->
 			<input type="button" id = "add" value="添加记录" class="button" onclick="addTr()">
 			<input type="button" id = "delete" value="撤销记录" class="button" onclick="deteleTr()">
 		</c:if>	
 		</span>
		<br><br>
		<center>
		...
		<form action="${pageContext.request.contextPath}/eveningpforrController.do?getSerialAndSaveTablet" onsubmit="return myvalidate()" method="post">
		<input id="id" name="id" type="hidden" value="${id}">
		<input type = "hidden" value = "${morningpforrEntity.id}" name = "morningpforrid" id = "morningpforrid">
		付款人:<input style="width:100px;" type="text" id = "paymen" name="paymen" value="${clientele}" required>
		类型 ：<select id="size" name="size"  onchange="changMoney();">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${morningpforrEntitys}" var="morningpforrEntity" varStatus="stauts">
				<tr>
					<td><input style="width:20px;"  type="checkbox"/></td>
					<!-- 付款人 
				 	<td><input style="width:50px;" type="text" id = "prayingobj1" name="prayingobj1" value="${morningpforrEntity.prayingobj}"></td>-->
				 	<!-- 阳上 -->
				 	<td><input style="width:60px;" type="text" id = "prayingobj2" name="prayingobj" value="${morningpforrEntity.prayingobj}"></td>
				 	<!-- 敬设 -->
				 	<td><input style="width:80px;" type="text" id="book" name="book" value="${morningpforrEntity.book}"></td>
				 	<!-- 公历 -->
				 	<td><input type="date" id="solardate"  name="solardate" value="${morningpforrEntity.solardate}" onchange="changeSolarToLunar(this)">
				 	</td>
				 	<!-- 农历 -->
				 	<td><input  type="text" id="lunardate" name="lunardate" value="${morningpforrEntity.lunardate}" placeholder="选择公历自动生成农历"><br>
				 	随堂：<label id="stCount">${counts[stauts.index].st}</label>&nbsp;&nbsp;&nbsp;&nbsp;包堂：<label id="btCount">${counts[stauts.index].bt}</label>
				 	
				 	</td>
				 	<!-- 历代宗亲 -->
				 	<td><input style="width:80px;" type ="text" name = "surname" id = "surname" value = "${surnames[stauts.index]}">门堂上历代宗亲</td>
				 	<!-- 超度对象 -->
				 	<td>
				 	是否有十方法界：<select style="width:100px;" id="type" name="type" font-size:20px;" >
								<option value="0" ${tmptypes[stauts.index] == 0 ? 'selected = "selected"' : '' }>无</option>
								<option value="1" ${tmptypes[stauts.index] == 1 ? 'selected = "selected"' : '' }>十方法界</option>
						</select><br><br>
				 	<input onclick="getAncestors(this);" style="width:200px;" type="text" name="ancestor" class="ancestor" value="${ancestorList[stauts.index]}"></td>
				 	<!-- 地址 -->
				 	<td><input  type="text" id = "address" name="address" value="${morningpforrEntity.address}"></td>
<%-- 				 	<td><input  type="text" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress:morningpforrEntity.address}"></td> --%>
				 	<!-- 金额 -->
				 	<td><input style="width:40px;" type="text" id = "money" name="money" value="600"  required></td>
				 	<!-- 付款方式 -->
				 	<td>
					...
					<select style="width:50px;" id="size" name="size" onchange="changeSize(this.value);">
				 	...
				 	<!-- 摘要 -->
				 	<td><textarea rows="3" cols="40" id="summary" name="summary"  onclick="changSumary();">${morningpforrEntity.summary}</textarea>
					</td>
    	...
    	<center>
			...
			<table id = "receipttable" class="imagetable">
				...
				<tr>
						<td><input style="width:20px;"  type="checkbox"/></td>
					 	<td><input style="width:50px;" type="text" id = "prayingobj" name="prayingobj" value="${clientele}"></td>
					 	<%-- <td><input style="width:100px;" type="text" id = "prayingobj2" name="prayingobj2" value="${clientele}"></td> --%>
					 	<td><input style="width:60px;" type="text" id = "book" name="book" value="往生普佛一堂" required>
					 	<!-- 公历 -->
				 		<td><input type="date" id="solardate"  name="solardate" value="" onchange="changeSolarToLunar(this)"></td>
				 		<!-- 农历 -->
				 		<td><input  type="text" id="lunardate" name="lunardate" value="" placeholder="选择公历自动生成农历"><br>
				 		随堂：<label id="stCount"></label>&nbsp;&nbsp;&nbsp;&nbsp;包堂：<label id="btCount"></label>
				 		</td>
				 		<td><input style="width:80px;" type ="text" name = "surname" id = "surname" value = "">门堂上历代宗亲</td>
					 	<td>
					 		<!-- <input onclick="getAncestors();" type="text" name="ancestor" id = "ancestor"> -->
					 		是否有十方法界：<select style="width:100px;" id="type" name="type" font-size:20px;" >
										<option value="0">无</option>
										<option value="1">十方法界</option>
									</select><br><br>
				 			<input onclick="getAncestors(this);" style="width:200px;" type="text" name="ancestor" id="ancestor" value="">
					 	</td>
					 	<td><input type="text" id = "address" name="address" value="${doritualinfoAddress}"></td>
					 	<td><input style="width:40px;" type="text" id = "money" name="money" value="600" required></td>
					 	<td>
					 	<select style="width:60px;" id="payway" name="payway" >
						<option value="现金">现金</option>
						...
						<select style="width:50px;" id="size" name="size" onchange="changeSize(this.value);">
					 	...
					 	<td><textarea rows="3" cols="40" id="summary" name="summary"  onclick="changSumary();"></textarea>
						</td>
						<%-- 	<td><input type="button" value="点击" onclick="LinktoPharmacistbirth('${receipt.ritualid}')"></td> --%>
