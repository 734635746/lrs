  ...
  <script type="text/javascript"> 
	...
	//获取复选框选中的值
  	...
  	$(function(){
 	    ...
 	    var tombsweepfesid = document.getElementById("tombsweepfesid").value;
	   	 if(tombsweepfesid != null){
 	...
 	function change(number,current){	//8月11号修改
  		...
  		if(number == "3"){
  			td.innerHTML = "<input style='width:30px;' type='text' name='ancestor' id = 'ancestor' required><br>门堂历代祖先";
  		} 
  		if(number == "4"){
...
;
 	...
 	function addTr(){
 		...
 		if(tr[1] == null){
 			...
 			//console.log(tr[1]);
 			//console.log(tr[1].children[8].children[0]);
 			tr[1].children[8].children[0].textContent = '';
 			//$(tr[0]children[8]).text('');
 	...
 	function changSumary(currentNode){
		...
		if(index == 0){//十方法界
			if("${fn:length(funeralheld)}" == "0"){
				...
				//currentNode.textContent = "农历${funeralheld[0].holdDate}举办清明节法会。十方法界";
				currentNode.textContent = "十方法界";
			}else{
				alert("清明节日期安排记录数必须有且仅有一条！");
			}
			
		}else if(index == 1){//门堂上历代祖先
			var ancestor = currentNode.parentNode.parentNode.children[3].children[0].value;
			//console.log(ancestor);
			currentNode.textContent = ancestor + "门堂上历代祖先";
		}else if(index == 2){//堕胎婴灵
			var ancestor = currentNode.parentNode.parentNode.children[3].children[0].value;
			currentNode.textContent =  ancestor + "堕胎婴灵";
		}else//无
			...
			<c:forEach items="${tombsweepfesEntitys}" var="tombsweepfesEntity" varStatus="stauts">
				<tr>
					<td><input style="width:20px;"  type="checkbox"/></td>
				 	...
				 	<td><input style="width:40px;" type="text" id = "money" name="money" value="${tombsweepfesEntity.money}" required></td>
				 	<td>
					...
					<select style="width:50px;" id="size" name="size" >
				 	...
				 	<td><textarea rows="3" cols="60" id="summary" name="summary" required>${tombsweepfesEntity.summary}</textarea>
					</td>
			...
			<table id = "receipttable" class="imagetable">
				...
				<tr>
						<td><input style="width:20px;"  type="checkbox"/></td>
					 	...
					 	<td onclick="getSelected(this)"><input type="text" name="ancestor" id = "ancestor" value=""></td>
