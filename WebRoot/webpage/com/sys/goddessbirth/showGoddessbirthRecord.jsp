 ...
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	function toGoddessbirth(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/goddessbirthController.do?redirectToEditGoddessbirth&id=" + id0 + "&size=" + encodeURI(size);
  	}
  	...
  	function selectBox(){
		 ...
		 $("[name='checkbox']:checked").each(function(){
			 goddessbirthIds += $(this).val()+",";
			 count = count + 1;
		  });
		 if(document.getElementById("size").value == "小"){
			 if(goddessbirthIds == "" || count > 5){
				 alert("请选择至少一个牌位，至多5个牌位");
				 return false;
			 }
			 else{
				 var id0 = document.getElementById("id").value;
				 window.location.href="${pageContext.request.contextPath}/goddessbirthController.do?redirectToEditGoddessbirth&id=" + id0 + "&goddessbirthIds=" + goddessbirthIds + "&size=" + encodeURI(size);
			 }	 
		 }
		 else{
			 if(goddessbirthIds == "" || count > 1){
				 alert("请选择一个牌位");
				 return false;
			 }
			 else{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${goddessbirthEntitys}" var="goddessbirthEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${goddessbirthEntity.id}" name="checkbox"></td>
					<td>${stauts.index+1}</td>
					<td>${goddessbirthEntity.paymen}</td>
					<td>${goddessbirthEntity.serial}</td>
					<td>${goddessbirthEntity.size}</td>
					<%-- <td>${goddessbirthEntity.receiptno}</td> --%>
				 	<td>${goddessbirthEntity.registertime}</td>
				 	<td>${goddessbirthEntity.prayingobj}</td>
				 	<td>${goddessbirthEntity.livingmenber}</td>
				 	<td>${goddessbirthEntity.address}</td>
				 	<td>${goddessbirthEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty goddessbirthEntitys}">
			<span id="recordArea">没有观音诞记录，请点击<input type="button" value="观音诞登记" class="button" onclick="toGoddessbirth()">&nbsp;&nbsp;</span>
			</c:if>
		</center>
 </body>