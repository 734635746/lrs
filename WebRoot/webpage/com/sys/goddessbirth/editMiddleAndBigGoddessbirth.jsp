...
#form h5{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		<h4>编辑观音诞${size}牌记录</h4><input type="button" value="预览" class="button" onclick = "preview()">
		...
		<form action="${pageContext.request.contextPath}/goddessbirthController.do?getSerialAndSaveTablet" onsubmit="return myvalidate()" method="post">
		<input id="id" name="id" type="hidden" value="${id}">
		<input type = "hidden" value = "${goddessbirthid}" name = "goddessbirthid" id = "goddessbirthid">
		...
		<td>
			...
			<tr>
				<th nowrap>付款人</th>
				<td><input style="width:100px;" type="text" id = "paymen" name="paymen" value="${goddessbirthEntity.paymen}" required> 
				</td>
			</tr>
			<tr>
				<th>敬设</th>
				<td><input style="width:150px;" type="text" id = "book" name="book" value="《梁皇宝忏》七昶日" required> 
				</td>
			</tr>
			<tr>
				<th>祈福者</th>
				<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${goddessbirthEntity.prayingobj}" required> 
				</td>
			</tr>
			<tr>
				<th>付款方式</th>
				<td> <select style="width:60px;" id="payway" name="payway" >
								<option value="现金" ${goddessbirthEntity.payway == '现金' ? 'selected = "selected"' : '' }>现金</option>
								<option value="刷卡" ${goddessbirthEntity.payway == '刷卡' ? 'selected = "selected"' : '' }>刷卡</option>
								<option value="支付宝" ${goddessbirthEntity.payway == '支付宝' ? 'selected = "selected"' : '' }>支付宝</option>
								<option value="微信" ${goddessbirthEntity.payway == '微信' ? 'selected = "selected"' : '' }>微信</option>
								<option value="其他" ${goddessbirthEntity.payway == '其他' ? 'selected = "selected"' : '' }>其他</option>
						</select>
				</td>
			</tr>
			<tr>
				<th>地址</th>
				<td> <input type="text" style="width:400px;" id = "address" name="address" value="${goddessbirthEntity.address}"></td>
<%-- 				<td> <input type="text" style="width:250px;" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress:goddessbirthEntity.address}"></td> --%>
			</tr>
			<tr>
				<th>金额</th>
				<td> <input type="text" style="width:60px;" id = "money" name="money" value="${goddessbirthEntity.money}" required onclick="changMoney();"></td>
			</tr>
			<tr>
				<th>摘要</th>
				<td> <textarea rows="3" cols="60" id="summary" name="summary"  onclick="changSumary()" autofocus required>${goddessbirthEntity.summary}</textarea></td>
			</tr>
		...
		<td>
			...
			<tr>
				<th>金额</th>
				<td> <input type="text" style="width:60px;" id = "money" name="money" value="" required required onclick="changMoney();"></td>
			</tr>
			<tr>
				<th>摘要</th>
