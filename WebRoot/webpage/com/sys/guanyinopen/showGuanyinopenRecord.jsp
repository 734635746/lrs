 ...
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	function toGuanyinopen(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/guanyinopenController.do?redirectToEditGuanyinopen&id=" + id0 + "&size=" + encodeURI(size);
  	}
  	...
  	function selectBox(){
		 ...
		 $("[name='checkbox']:checked").each(function(){
			 guanyinopenIds += $(this).val()+",";
			 count = count + 1;
		  });
		 
		 if(document.getElementById("size").value == "小"){
			 if(guanyinopenIds == "" || count > 5){
				 alert("请选择至少一个牌位，至多5个牌位");
				 return false;
			 }
			 else{
				 var id0 = document.getElementById("id").value;
				 window.location.href="${pageContext.request.contextPath}/guanyinopenController.do?redirectToEditGuanyinopen&id=" + id0 + "&guanyinopenIds=" + guanyinopenIds + "&size=" + encodeURI(size);
			 }	 
		 }
		 else{
			 if(guanyinopenIds == "" || count > 1){
				 alert("请选择一个牌位");
				 return false;
			 }
			 else{
...
//祈福类法事，通过ajax查询共享数据 
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${guanyinopenEntitys}" var="guanyinopenEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${guanyinopenEntity.id}" name="checkbox"></td>
					<td>${stauts.index+1}</td>
					<td>${guanyinopenEntity.paymen}</td>
					<td>${guanyinopenEntity.serial}</td>
					<td>${guanyinopenEntity.size}</td>
					<%-- <td>${guanyinopenEntity.receiptno}</td> --%>
				 	<td>${guanyinopenEntity.registertime}</td>
				 	<td>${guanyinopenEntity.prayingobj}</td>
				 	<td>${guanyinopenEntity.livingmenber}</td>
				 	<td>${guanyinopenEntity.address}</td>
				 	<td>${guanyinopenEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty guanyinopenEntitys}">
			<span id="recordArea">没有观音开库记录，请点击<input type="button" value="观音开库登记" class="button" onclick="toGuanyinopen()">&nbsp;&nbsp;</span>
			</c:if>
		</center>
 </body>