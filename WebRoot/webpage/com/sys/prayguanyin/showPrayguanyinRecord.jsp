...
<html>
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	/* function toMorningPforr(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/prayguanyinController.do?ToEditPrayGuanyin&id=" + id0 + "&rname=" + rname + "&phonenumber1=" + phonenumber1 + "&phonenumber2=" + phonenumber2 + "&address=" + address;
  	} */
  	...
  	function toMorningPforr(){
  		...
  		if(param == undefined || param == ''){
	  		...
	  		window.location.href = "${pageContext.request.contextPath}/prayguanyinController.do?ToEditPrayGuanyin" + param;
  		}
  	...
  	function selectBox(){
		 ...
		 if(morningPforrId == "" || count > 5){
			 ...
			 //var id0 = document.getElementById("id").value;
			 window.location.href="${pageContext.request.contextPath}/prayguanyinController.do?ToEditPrayGuanyin" + "&morningPforrId=" + morningPforrId+param;
		 }
	}
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<center>
		<h4>上供记录</h4>
		<c:if test="${!empty morningpforrEntityList}">
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${morningpforrEntityList}" var="morningpforrEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${morningpforrEntity.id}" name="checkbox"></td>
					<td>${stauts.index+1}</td>
					<td>${morningpforrEntity.paymen}</td>
					<td>${morningpforrEntity.serial}</td>
					<td>${morningpforrEntity.size}</td>
					<td>${morningpforrEntity.receiptno}</td>
				 	<%-- <td>${morningpforrEntity.registertime}</td> --%>
				 	<%-- <td>${morningpforrEntity.prayingobj}</td> --%>
				 	<td>${morningpforrEntity.livingmenber }</td>
				 	<td>${morningpforrEntity.address}</td>
				 	<td>${morningpforrEntity.lunardate}</td>
				 	<td>${morningpforrEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty morningpforrEntityList}">
			<tr>没有上供记录，请点击<input type="button" value="上供登记" class="button" onclick="toMorningPforr()">&nbsp;&nbsp;</tr>
			</c:if>
		</center>
 </body>