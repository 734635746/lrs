...
<html>
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	function toGuanyingaya(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/guanyingayaController.do?redirectToEditGuanyingaya&id=" + id0 + "&size=" + encodeURI(size);
  	}
  	
  	function LinktoPharmacistbirth(ritualid){
  		var id = document.getElementById("id").value;
  		window.location.href = "${pageContext.request.contextPath}/guanyingayaController.do?redirectToPharmacistbirthByRitualid&id=" + id + "&ritualid=" + ritualid;
  	}
  	
  	/* function selectBox(){
	  		$("#receipttable :checkbox").each(function(){
  				if(this.checked){
  					var input0 = $.trim($(this).parent().parent().find("input[hidden='true']").val());
  	...
  	function selectBox(){
		var count = 0;
		 ...
		 $("[name='checkbox']:checked").each(function(){
			 guanyingayaIds += $(this).val()+",";
			 count = count + 1;
		  });
		 if(document.getElementById("size").value == "小"){
			 if(guanyingayaIds == "" || count > 5){
				 alert("请选择至少一个牌位，至多5个牌位");
				 return false;
			 }
			 else{
				 var id0 = document.getElementById("id").value;
				 window.location.href="${pageContext.request.contextPath}/guanyingayaController.do?redirectToEditGuanyingaya&id=" + id0 + "&guanyingayaIds=" + guanyingayaIds + "&size=" + encodeURI(size);
			 }	 
		 }
		 else{
			 if(guanyingayaIds == "" || count > 1){
				 alert("请选择一个牌位");
				 return false;
			 }
			 else{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<br>
		<c:if test="${!empty guanyingayaEntitys}">
		<span id="recordArea">
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${guanyingayaEntitys}" var="guanyingayaEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${guanyingayaEntity.id}" name="checkbox"></td>
					<td>${stauts.index+1}</td>
					<td>${guanyingayaEntity.serial}</td>
					<td>${guanyingayaEntity.paymen}</td>
					<td>${guanyingayaEntity.size}</td>
					<%-- <td>${guanyingayaEntity.receiptno}</td> --%>
				 	<td>${guanyingayaEntity.registertime}</td>
				 	<td>${guanyingayaEntity.prayingobj}</td>
				 	<td>${guanyingayaEntity.livingmenber}</td>
				 	<td>${guanyingayaEntity.address}</td>
				 	<td>${guanyingayaEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty guanyingayaEntitys}">
			<span id="recordArea">没有观音成道记录，请点击<input type="button" value="观音成道登记" class="button" onclick="toGuanyingaya()">&nbsp;&nbsp;</span>
			</c:if>
		</center>
 </body>