...
#form h5{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		<h4>编辑盂兰节${size}牌记录</h4><input type="button" value="预览" class="button" onclick = "preview()">
		...
		<form action="${pageContext.request.contextPath}/ghostfesController.do?getSerialAndSaveTablet" onsubmit="return myvalidate()" method="post">
		<input id="id" name="id" type="hidden" value="${id}">
		<input id="size" name="sizeflag" type="hidden" value="${size}">
		<input type = "hidden" value = "${ghostfesid}" name = "ghostfesid" id = "ghostfesid">
		...
		<td>
			...
			<tr>
				<th>付款人</th>
				<td><input style="width:100px;" type="text" id = "paymen" name="paymen" value="${ghostfesEntity.paymen}" required> 
				</td>
			</tr>
			<tr>
				<th>敬设</th>
			...
			<tr>
				<th>阳上</th>
				<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${ghostfesEntity.prayingobj}" required>
				</td>
			</tr>
			<tr>
				<th>付款方式</th>
				<td> <select style="width:60px;" id="payway" name="payway" >
								<option value="现金" ${ghostfesEntity.payway == '现金' ? 'selected = "selected"' : '' }>现金</option>
								<option value="刷卡" ${ghostfesEntity.payway == '刷卡' ? 'selected = "selected"' : '' }>刷卡</option>
								<option value="支付宝" ${ghostfesEntity.payway == '支付宝' ? 'selected = "selected"' : '' }>支付宝</option>
								<option value="微信" ${ghostfesEntity.payway == '微信' ? 'selected = "selected"' : '' }>微信</option>
								<option value="其他" ${ghostfesEntity.payway == '其他' ? 'selected = "selected"' : '' }>其他</option>
						</select>
				</td>
			</tr>
			<tr>
				<th>地址</th>
				<td> <input type="text" style="width:400px;" id = "address" name="address" value="${ghostfesEntity.address}"></td>
<%-- 				<td> <input type="text" style="width:250px;" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress:ghostfesEntity.address}"></td> --%>
			</tr>
			<%-- <input type="hidden" style="width:250px;" id = "address" name="address" value="${doritualinfoAddress}"> --%>
			<tr>
				<th>金额</th>
				<td> <input type="text" style="width:60px;" id = "money" name="money" value="${ghostfesEntity.money}" required onclick="changMoney();"></td>
			</tr>
			<tr>
				<th>摘要</th>
				<td> <textarea rows="3" cols="60" id="summary" name="summary"  onclick="changSumary()" autofocus required>${ghostfesEntity.summary}</textarea></td>
			</tr>
