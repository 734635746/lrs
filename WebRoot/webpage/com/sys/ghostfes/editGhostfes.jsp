			...
			<c:forEach items="${ghostfesEntitys}" var="ghostfesEntity" varStatus="stauts">
				<tr>
					<td><input style="width:20px;"  type="checkbox"/></td>
				 	<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${ghostfesEntity.prayingobj}"></td>
				 	<td><select style="width:100px;" id="type" name="type" onchange="change(this.value,this)">
						<option value="2" ${ghostfesEntity.type == 2 ? 'selected = "selected"' : '' }>十方法界</option>
						<option value="3" ${ghostfesEntity.type == 3 ? 'selected = "selected"' : '' }>门堂历代宗亲</option>
						<option value="4" ${ghostfesEntity.type == 4 ? 'selected = "selected"' : '' }>堕胎婴灵</option>
						<option value="1" ${ghostfesEntity.type == 1 ? 'selected = "selected"' : '' }>无</option>
					</select>
					</td>
				 	<td onclick="getSelected(this)"><input type="text" name="ancestor" id = "ancestor" value="${ghostfesEntity.ancestor}"></td>
				 	<input type="hidden" id = "address" name="address"  value="${ghostfesEntity.address}">
<%-- 				 	<input type="hidden" id = "address" name="address"  value="${updateFlag!=1? doritualinfoAddress:ghostfesEntity.address}"> --%>
				 	<%-- <td><input type="hidden" id = "address" name="address" value="${ghostfesEntity.address}" ></td> --%>
					...
					<select style="width:50px;" id="size" name="size" >
				 	...
				 	<td><textarea rows="3" cols="60" id="summary" name="summary" required onclick="changSumary(this);">${ghostfesEntity.summary}</textarea>
					</td>
			...
			<table id = "receipttable" class="imagetable">
				...
				<tr>
						<td><input style="width:20px;"  type="checkbox"/></td>
					 	...
					 	<!-- <td><input type="hidden" id = "address" name="address" value="" ></td> -->
					 	<input type="hidden" id = "address" name="address" value="${doritualinfoAddress}" >
					 	<td><input style="width:40px;" type="text" id = "money" name="money" value="100"></td>
					 	<td>
					 	<select style="width:60px;" id="payway" name="payway" >
						<option value="现金">现金</option>
						<option value="刷卡">刷卡</option>
