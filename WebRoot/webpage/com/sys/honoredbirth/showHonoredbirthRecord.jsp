 ...
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	function toHonoredbirth(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/honoredbirthController.do?redirectToEditHonoredbirth&id=" + id0 + "&size=" + encodeURI(size);
  	}
  	...
  	function selectBox(){
		var count = 0;
		 ...
		 $("[name='checkbox']:checked").each(function(){
			 honoredbirthIds += $(this).val()+",";
			 count = count + 1;
		  });
		 
		 if(document.getElementById("size").value == "小"){
			 if(honoredbirthIds == "" || count > 5){
				 alert("请选择至少一个牌位，至多5个牌位");
				 return false;
			 }
			 else{
				 var id0 = document.getElementById("id").value;
				 window.location.href="${pageContext.request.contextPath}/honoredbirthController.do?redirectToEditHonoredbirth&id=" + id0 + "&honoredbirthIds=" + honoredbirthIds + "&size=" + encodeURI(size);
			 }	 
		 }
		 else{
			 if(honoredbirthIds == "" || count > 1){
				 alert("请选择一个牌位");
				 return false;
			 }
			 else{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${honoredbirthEntitys}" var="honoredbirthEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${honoredbirthEntity.id}" name="checkbox"></td>
					<td>${stauts.index+1}</td>
					<td>${honoredbirthEntity.paymen}</td>
					<td>${honoredbirthEntity.serial}</td>
					<td>${honoredbirthEntity.size}</td>
					<%-- <td>${honoredbirthEntity.receiptno}</td> --%>
				 	<td>${honoredbirthEntity.registertime}</td>
				 	<td>${honoredbirthEntity.prayingobj}</td>
				 	<td>${honoredbirthEntity.livingmenber}</td>
				 	<td>${honoredbirthEntity.address}</td>
				 	<td>${honoredbirthEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty honoredbirthEntitys}">
			<span id="recordArea">没有释尊诞记录，请点击<input type="button" value="释尊诞登记" class="button" onclick="toHonoredbirth()">&nbsp;&nbsp;</span>
			</c:if>
		</center>
 </body>