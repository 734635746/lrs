 ...
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	function toBuddhabirth(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/buddhabirthController.do?redirectToEditBuddhabirth&id=" + id0 + "&size=" + encodeURI(size);
  	}
  	
  	function selectBox(){ //10月29号做了修改
  		 var count = 0;
		 var buddhabirthIds = "";
		 ...
		 $("[name='checkbox']:checked").each(function(){
			 ...
			 buddhabirthIds += $(this).val() + ",";
			 count = count + 1;
		  });
		 if(document.getElementById("size").value == "小"){
			 if(buddhabirthIds == "" || count > 5){
				 alert("请选择至少一个牌位，至多5个牌位");
				 return false;
			 }
			 else{
				 var id0 = document.getElementById("id").value;
				 window.location.href="${pageContext.request.contextPath}/buddhabirthController.do?redirectToEditBuddhabirth&id=" + id0 + "&buddhabirthIds=" + buddhabirthIds + "&size=" + encodeURI(size);
			 }	 
		 }
		 else{
			 if(buddhabirthIds == "" || count > 1){
				 alert("请选择一个牌位");
				 return false;
			 }
			 else{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${buddhabirthEntitys}" var="buddhabirthEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${buddhabirthEntity.id}" name="checkbox"></td> 
					<td>${stauts.index+1}</td>
					<td>${buddhabirthEntity.paymen}</td>
					<td>${buddhabirthEntity.serial}</td>
					<td>${buddhabirthEntity.size}</td>
					<%-- <td>${buddhabirthEntity.receiptno}</td> --%>
				 	<td>${buddhabirthEntity.registertime}</td>
				 	<td>${buddhabirthEntity.prayingobj}</td>
				 	<td>${buddhabirthEntity.livingmenber}</td>
				 	<td>${buddhabirthEntity.address}</td>
				 	<td>${buddhabirthEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty buddhabirthEntitys}">
			<span id="recordArea">没有弥勒佛诞记录，请点击<input type="button" value="弥勒佛诞登记" class="button" onclick="toBuddhabirth()">&nbsp;&nbsp;</span>
			</c:if>
		</center>
 </body>