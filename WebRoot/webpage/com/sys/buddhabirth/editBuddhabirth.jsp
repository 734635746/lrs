	...
	//获取复选框选中的值
  	...
  	$(function(){ //10月29号改
 	     ...
 	     $("#SelectAncestorAll").click(function(){
 	   	...
 	   	var buddhabirthid = document.getElementById("buddhabirthid").value;
	   	 if(buddhabirthid != null){
...
#form h5{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<c:if test="${!empty buddhabirthEntitys}">
		...
		<input type = "hidden" value = "${buddhabirthEntity.id}" name = "buddhabirthid" id = "buddhabirthid">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${buddhabirthEntitys}" var="buddhabirthEntity" varStatus="stauts">
				<tr>
					<td><input style="width:20px;"  type="checkbox"/></td>
				 	<td><input style="width:100px;" type="text" id = "prayingobj" name="prayingobj" value="${buddhabirthEntity.prayingobj}"></td>
				 	<td onclick="getSelected(this)"><input type="text" name="livingmenber" id = "livingmenber" value="${buddhabirthEntity.livingmenber}"></td>
				 	<%-- <td><input type="text" id = "address" name="address" value="${buddhabirthEntity.address}"></td> --%>
				 	<input type="hidden" id = "address" name="address" value="${buddhabirthEntity.address}">
<%-- 				 	<input type="hidden" id = "address" name="address" value="${updateFlag!=1? doritualinfoAddress:buddhabirthEntity.address}"> --%>
				 	<td><input style="width:40px;" type="text" id = "money" name="money" value="${buddhabirthEntity.money}" required></td>
				 	<td>
					...
					<select style="width:50px;" id="size" name="size" onchange="changeSize(this.value);">
						<option value="小" <c:if test="${buddhabirthEntity.size eq '小'}">selected="selected"</c:if>>小</option>
					</select>
					</td>
				 	
				 	<td><textarea rows="3" cols="60" id="summary" name="summary" required>${buddhabirthEntity.summary}</textarea>
					</td>
				...
				<tr>
					 ...
					 <!-- 	<td><input type="text" id = "address" name="address" value=""></td> -->
						...
						<select style="width:50px;" id="size" name="size" onchange="changeSize(this.value);">
					 	...
					 	<td><textarea rows="3" cols="60" id="summary" name="summary" required></textarea>
						</td>
						<%-- 	<td><input type="button" value="点击" onclick="LinktoPharmacistbirth('${receipt.ritualid}')"></td> --%>
