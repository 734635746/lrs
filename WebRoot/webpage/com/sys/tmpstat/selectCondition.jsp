...
function getData(){
	...
	var departid = document.getElementById("depart").options[document.getElementById("depart").selectedIndex].value;
	var memberName = document.getElementById("member").options[document.getElementById("member").selectedIndex].value;
	
	var url = "tmp_statController.do?statResult&id=" + encodeURIComponent(encodeURIComponent(departid)) + "&memberName=" + encodeURIComponent(encodeURIComponent(memberName)) + "&start=" + start + "&end=" + end;
	$.ajax({
		type : 'POST',
		url : url,
		success : function(data) {
