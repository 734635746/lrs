 ...
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/button.css">
  	...
  	function toBuddhagaya(){
  		...
  		window.location.href = "${pageContext.request.contextPath}/buddhagayaController.do?redirectToEditBuddhagaya&id=" + id0 + "&size=" + encodeURI(size);
  	}
  	...
  	function selectBox(){
		 ...
		 $("[name='checkbox']:checked").each(function(){
			 buddhagayaIds += $(this).val()+",";
			 count = count + 1;
		  });
		 if(document.getElementById("size").value == "小"){
			 if(buddhagayaIds == "" || count > 5){
				 alert("请选择至少一个牌位，至多5个牌位");
				 return false;
			 }
			 else{
				 var id0 = document.getElementById("id").value;
				 window.location.href="${pageContext.request.contextPath}/buddhagayaController.do?redirectToEditBuddhagaya&id=" + id0 + "&buddhagayaIds=" + buddhagayaIds + "&size=" + encodeURI(size);
			 }	 
		 }
		 else{
			 if(buddhagayaIds == "" || count > 1){
				 alert("请选择一个牌位");
				 return false;
			 }
			 else{
 ...
 <body style="overflow-y: scroll" scroll="yes">
		...
		<table id = "receipttable" class="imagetable">
			...
			<c:forEach items="${buddhagayaEntitys}" var="buddhagayaEntity" varStatus="stauts">
				<tr>
					<td><input type="checkbox" value="${buddhagayaEntity.id}" name="checkbox"></td>
					<td>${stauts.index+1}</td>
					<td>${buddhagayaEntity.paymen}</td>
					<td>${buddhagayaEntity.serial}</td>
					<td>${buddhagayaEntity.size}</td>
					<%-- <td>${buddhagayaEntity.receiptno}</td> --%>
				 	<td>${buddhagayaEntity.registertime}</td>
				 	<td>${buddhagayaEntity.prayingobj}</td>
				 	<td>${buddhagayaEntity.livingmenber}</td>
				 	<td>${buddhagayaEntity.address}</td>
				 	<td>${buddhagayaEntity.summary}</td>
		...
		<br><br>
		<c:if test="${empty buddhagayaEntitys}">
			<span id="recordArea">没有释尊成道记录，请点击<input type="button" value="释尊成道登记" class="button" onclick="toBuddhagaya()">&nbsp;&nbsp;</span>
			</c:if>
		</center>
 </body>