...
public final class ExcelExportUtil {
	 ...
	 * @return
	 */
	public static HSSFCellStyle getStatusStyle(HSSFWorkbook workbook, boolean isWarp, short bg) {
		HSSFCellStyle style = workbook.createCellStyle();
		style.setBorderLeft((short) 1); // 左边框
		style.setBorderRight((short) 1); // 右边框
		style.setBorderBottom((short) 1);
		style.setBorderTop((short) 1);
		style.setFillForegroundColor(bg); // 填充的背景颜色
		//style.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND); // 填充图案
		style.setAlignment(HSSFCellStyle.ALIGN_CENTER);
		style.setVerticalAlignment(HSSFCellStyle.VERTICAL_CENTER);
		if(isWarp){style.setWrapText(true);}
	 ...
	 * @return
	 */
	public static HSSFWorkbook exportStatusExcel(ExcelTitle entity,
			Class<?> pojoClass, Collection<?> dataSet, int columnCount) {
		HSSFWorkbook workbook = new HSSFWorkbook();
		createSheetInUserModel2FileStatus(workbook, entity, pojoClass, dataSet, columnCount);
		return workbook;
	}
	/**
	 * 很据模型类导出状态表
	 ...
	 * @param dataSet
	 */
	private static void createSheetInUserModel2FileStatus(HSSFWorkbook workbook,
			ExcelTitle entity, Class<?> pojoClass, Collection<?> dataSet, int columnCount) {
		try {
			Sheet sheet = workbook.createSheet(entity.getSheetName());
			//创建表格属性
			Map<String,HSSFCellStyle> styles = createStatusStyles(workbook);
			Drawing patriarch = sheet.createDrawingPatriarch();
			List<ExcelExportEntity> excelParams = new ArrayList<ExcelExportEntity>();
			// 得到所有字段
			Field fileds[] = ExcelPublicUtil.getClassFields(pojoClass);
			...
			//int feildWidth = getFieldWidth(excelParams);
			if (entity.getTitle() != null) {
				int i =  createHeaderRow(entity, sheet, workbook, columnCount);
				sheet.createFreezePane(0, 2+i, 0, 2+i);
				index += i;
			} else {
				sheet.createFreezePane(0, 2, 0, 2);
			...
			createStatusTitleRow(entity,sheet, workbook, index, excelParams, columnCount);
			index += 1;
			setCellWith(excelParams, sheet);
			Iterator<?> its = dataSet.iterator();
			while (its.hasNext()) {
				Object t = its.next();
				index += createStatusCells(patriarch,index, t, excelParams, sheet, workbook
						,styles, columnCount);
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @throws Exception
	 */
	private static int createStatusCells(Drawing patriarch, int index, Object t,
			List<ExcelExportEntity> excelParams, Sheet sheet,
			HSSFWorkbook workbook, Map<String, HSSFCellStyle> styles, int columnCount) throws Exception {
		ExcelExportEntity entity;
		Row row = sheet.createRow(index);
		row.setHeight((short) 350);
		int maxHeight = 1, cellNum = 0;
		for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
			...
			if (entity.getList() != null) {
				...
				if(value.toString().equals("1")){
					createStatusStringCell(row, cellNum++,
							/*value == null ? "" : value.toString()*/"", getStatusStyles(styles, false, entity.isWrap()),entity, columnCount);
				}else if(value.toString().equals("0")){
					createStatusStringCell(row, cellNum++,
							/*value == null ? "" : value.toString()*/"", getStatusStyles(styles, true, entity.isWrap()),entity, columnCount);
				}else{
					createStatusStringCell(row, cellNum++,
							value == null ? "" : value.toString(), getStatusStyles(styles, true, entity.isWrap()),entity, columnCount);
				}
	 ...
	 * @param excelParams
	 */
	private static void createStatusTitleRow(ExcelTitle title, Sheet sheet, HSSFWorkbook workbook,
			int index, List<ExcelExportEntity> excelParams,int columnCount) {
		Row row = sheet.createRow(index);
		//Row row1 = sheet.createRow(index + 1);
		row.setHeight((short) 450);
		int cellIndex = 0;
		...
		for (int i = 0, exportFieldTitleSize = excelParams.size(); i < exportFieldTitleSize; i++) {
			...
			createStatusStringCell(row, cellIndex, entity.getName(), titleStyle, entity, columnCount);
			/*if (entity.getList() != null) {
				List<ExcelExportEntity> sTitel = entity.getList();
				sheet.addMergedRegion(new CellRangeAddress(index, index,cellIndex,cellIndex
						+ sTitel.size() - 1));
...
//					cellIndex));
	...
	public static HSSFCellStyle getTwoStatusStyle(HSSFWorkbook workbook, boolean isWarp) {
		HSSFCellStyle style = workbook.createCellStyle();
		style.setBorderLeft((short) 1); // 左边框
		style.setBorderRight((short) 1); // 右边框
		style.setBorderBottom((short) 1);
	...
	public static HSSFCellStyle getOneStatusStyle(HSSFWorkbook workbook, boolean isWarp) {
		HSSFCellStyle style = workbook.createCellStyle();
		style.setBorderLeft((short) 1); // 左边框
		style.setBorderRight((short) 1); // 右边框
		style.setBorderBottom((short) 1);
	...
	private static Map<String, HSSFCellStyle> createStatusStyles(HSSFWorkbook workbook) {
		Map<String, HSSFCellStyle> map = new HashMap<String, HSSFCellStyle>();
		map.put("one", getOneStatusStyle(workbook,false));
		map.put("oneWrap", getOneStatusStyle(workbook,true));
		map.put("two", getTwoStatusStyle(workbook,false));
		map.put("twoWrap", getTwoStatusStyle(workbook,true));
		return map;
	}
	
	private static CellStyle getStatusStyles(Map<String, HSSFCellStyle>  map,boolean needOne,boolean isWrap) {
		if(needOne&&isWrap){
			return map.get("oneWrap");
		}
		if(needOne){
	...
	private static void createStatusStringCell(Row row, int index, String text,
			CellStyle style, ExcelExportEntity entity, int columnCount) {
