...
public class JeecgOneToMainUtil {
	...
	public static void main(String[] args) {
		...
		codeParamEntityIn.setTableName("memorial_tablet");//主表[表名]
		codeParamEntityIn.setEntityName("Memorial_tablet");	 //主表[实体名]
		codeParamEntityIn.setEntityPackage("memorial_tablet");	 //主表[包名]
		codeParamEntityIn.setFtlDescription("牌位表");	 //主表[描述]
		...
		po.setTableName("namelist");//子表[表名]
		po.setEntityName("Namelist");	 //子表[实体名]
		po.setEntityPackage("namelist");			 //子表[包]
		po.setFtlDescription("姓名列表");		 //子表[描述]
		//子表[外键:与主表关联外键]
		//说明：这里面的外键是子表的外键字段,非主表和子表的对应关系  GORDER_ID修改为ID
		po.setForeignKeys(new String[]{"tablet_id"});
		subTabParamIn.add(po);
		//[2].子表二
		SubTableEntity po2 = new SubTableEntity();
		po2.setTableName("linkmanlist");		//子表[表名]
		po2.setEntityName("Linkmanlist");			//子表[实体名]
		po2.setEntityPackage("linkmanlist"); 					//子表[包]
		po2.setFtlDescription("联系人列表");				//子表[描述]
		//子表[外键:与主表关联外键]
		//说明：这里面的外键是子表的外键字段,非主表和子表的对应关系      GORDER_ID修改为ID
		po2.setForeignKeys(new String[]{"tablet_id"});
